<template>
  <div class="inv-container">
    <div class="filter-bar">
      <el-input v-model="filterKeyword" placeholder="搜索药品名称" clearable size="small" class="filter-input" />
    ...
    <el-table :data="tableData" class="data-table" empty-text="暂无数据" v-loading="loading">
      ...
      <el-table-column prop="type" label="类别" min-width="100">
        <template #default="{ row }"><span class="type-tag">{{ row.type }}</span></template>
      </el-table-column>
      ...
      <el-table-column label="单价(元)" min-width="90">
        <template #default="{ row }">¥{{ row.unitPrice }}</template>
      </el-table-column>
      <el-table-column prop="manufactureDate" label="生产日期" min-width="110" />
      <el-table-column prop="expireDate" label="有效期至" min-width="110" />
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑药品' : '新增药品'" width="500px" class="add-dialog" :append-to-body="true" :close-on-click-modal="false">
      <el-form :model="form" label-width="90px">
        <el-form-item label="药品名称"><el-input v-model="form.medicineName" /></el-form-item>
        <el-form-item label="类别"><el-input v-model="form.type" placeholder="如: 消毒剂、抗生素" /></el-form-item>
        <el-form-item label="供应商"><el-input v-model="form.supplier" /></el-form-item>
        <el-form-item label="区域">
          <el-input v-model="form.zoneName" placeholder="如：药品柜-A层" />
        </el-form-item>
        <el-form-item label="库存"><el-input-number v-model="form.currentStock" :min="0" :precision="2" style="width:100%" /></el-form-item>
        <el-form-item label="预警阈值"><el-input-number v-model="form.warningThreshold" :min="0" :precision="2" style="width:100%" /></el-form-item>
        ...
        <el-form-item label="生产日期"><el-date-picker v-model="form.manufactureDate" type="date" style="width:100%" value-format="YYYY-MM-DD" /></el-form-item>
        <el-form-item label="有效期至"><el-date-picker v-model="form.expireDate" type="date" style="width:100%" value-format="YYYY-MM-DD" /></el-form-item>
...
import { getMedicineInventories, createMedicineInventory, updateMedicineInventory, deleteMedicineInventory } from '@/api/record'
...
export default {
  ...
  setup() {
    ...
    const form = reactive({
      medicineName: '', type: '消毒剂', supplier: '', zoneName: '',
      currentStock: 0, warningThreshold: 0, unit: 'box',
      unitPrice: 0, manufactureDate: '', expireDate: '', remarks: ''
    })

    const resetForm = () => {
      form.medicineName = ''; form.type = '消毒剂'; form.supplier = ''; form.zoneName = '';
      form.currentStock = 0; form.warningThreshold = 0; form.unit = 'box';
      form.unitPrice = 0; form.manufactureDate = ''; form.expireDate = ''; form.remarks = ''
    }
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const res = await getMedicineInventories(params)
    ...
    const handleSave = async () => {
      ...
      try {
        const data = { ...form }
        if (editing.value) { await updateMedicineInventory(editing.value.id, data); ElMessage.success('编辑成功') }
        else { await createMedicineInventory(data); ElMessage.success('添加成功') }
    ...
    const handleDelete = async (row) => {
      try { await ElMessageBox.confirm('确定删除？', '提示', { type: 'warning' }); await deleteMedicineInventory(row.id); ElMessage.success('已删除'); loadData() } catch { /* ignore */ }
    }
...
.type-tag { background: rgba(80,150,220,0.1); color: #8cb8e0; padding: 2px 8px; border-radius: 6px; font-size: 12px; }
