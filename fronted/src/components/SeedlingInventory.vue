<template>
  <div class="inv-container">
    <div class="filter-bar">
      <el-input v-model="filterKeyword" placeholder="搜索苗种名称" clearable size="small" class="filter-input" />
    ...
    <el-table :data="tableData" class="data-table" empty-text="暂无数据" v-loading="loading">
      <el-table-column prop="seedlingName" label="苗种名称" min-width="150" />
      ...
      <el-table-column prop="hatchDate" label="孵化日期" min-width="110" />
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑苗种' : '新增苗种'" width="500px" class="add-dialog" :append-to-body="true" :close-on-click-modal="false">
      <el-form :model="form" label-width="90px">
        <el-form-item label="苗种名称"><el-input v-model="form.seedlingName" /></el-form-item>
        <el-form-item label="供应商"><el-input v-model="form.supplier" /></el-form-item>
        <el-form-item label="区域">
          <el-input v-model="form.zoneName" placeholder="如：A区-1号池" />
        </el-form-item>
        <el-form-item label="库存"><el-input-number v-model="form.currentStock" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="预警阈值"><el-input-number v-model="form.warningThreshold" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="单位">
          <el-select v-model="form.unit" style="width:100%">
            <el-option label="尾" value="tail" />
            <el-option label="万尾" value="wanwei" />
          </el-select>
        </el-form-item>
        <el-form-item label="单价"><el-input-number v-model="form.unitPrice" :min="0" :precision="4" style="width:100%" /></el-form-item>
        ...
        <el-form-item label="孵化日期"><el-date-picker v-model="form.hatchDate" type="date" style="width:100%" value-format="YYYY-MM-DD" /></el-form-item>
...
import { getSeedlingInventories, createSeedlingInventory, updateSeedlingInventory, deleteSeedlingInventory } from '@/api/record'
...
export default {
  ...
  setup() {
    ...
    const form = reactive({
      seedlingName: '', supplier: '', zoneName: '',
      currentStock: 0, warningThreshold: 0, unit: 'tail',
      unitPrice: 0, purchaseDate: '', hatchDate: '', remarks: ''
    })

    const resetForm = () => {
      form.seedlingName = ''; form.supplier = ''; form.zoneName = '';
      form.currentStock = 0; form.warningThreshold = 0; form.unit = 'tail';
      form.unitPrice = 0; form.purchaseDate = ''; form.hatchDate = ''; form.remarks = ''
    }
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const res = await getSeedlingInventories(params)
    ...
    const handleSave = async () => {
      ...
      try {
        const data = { ...form }
        if (editing.value) { await updateSeedlingInventory(editing.value.id, data); ElMessage.success('编辑成功') }
        else { await createSeedlingInventory(data); ElMessage.success('添加成功') }
    ...
    const handleDelete = async (row) => {
      try { await ElMessageBox.confirm('确定删除？', '提示', { type: 'warning' }); await deleteSeedlingInventory(row.id); ElMessage.success('已删除'); loadData() } catch { /* ignore */ }
    }
